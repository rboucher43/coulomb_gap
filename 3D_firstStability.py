...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 15 19:52:29 2020
...
lattice_x       = 15                       #stateNum = lattice_x*lattice_y*lattice_z
lattice_y       = 15
lattice_z       = 15 
w               = 1                       #Max and abs(min) energy 
realizationsNum = 100                     #Number of separate calculations
alpha           = 1                     #Raise potential to power alpha
...
while realizations < realizationsNum:
    ...
    while stop == False:
        ...
        rDiff = np.sqrt((highInd[0]-lowInd[0])**2+(highInd[1]-lowInd[1])**2+(highInd[2]-lowInd[2])**2) 
        ...
        if highVal == 0 and lowVal == 0:   
            ...
            j = 0
            complete = True
            while h < lattice_z and complete == True:
                while i < lattice_y and complete == True:        #First Order Stability
                    while j < lattice_x and complete == True:        
                        m = 0
                        k = 0
                        l = 0
                        while m < lattice_z and complete == True:
                            while k < lattice_y and complete == True:
                                while l < lattice_x and complete == True:                    
                                    if energies[m][k][l] > energies[h][i][j]  and N[h][i][j] == 1 and N[m][k][l] == 0 and energies[m][k][l]-energies[h][i][j] < (1/np.sqrt((m-h)**2+(k-i)**2+(l-j)**2)**alpha):
                                        highVal    = 1
                                        lowVal     = -1
                                        highInd[0] = h
                                        highInd[1] = i
...
np.save("3-D_15x15x15_100_r^1_first.npy",allEnergies)
...
plt.grid()
plt.savefig("3-D_15x15x15_100_r^1_first_DOS.pdf")
plt.show()
